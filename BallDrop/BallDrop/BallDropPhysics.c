...
//-----------------------------------------------------------------------------------
static void GetVectorDistFromBlock (float point[2], BallDropBlock *block, float *vectorDist);
static float GetSignedDistFromHalfPlane (float point[2], BallDropHalfPlane *halfPlane);
static float GetBallBallCollisionTime (BallDropBall *ballA, BallDropBall *ballB, float deltaT);
static float GetBallBlockCollisionTime (BallDropBall *ball, BallDropBlock *block, float deltaT);
//static float GetBallPlaneCollisionTime (BallDropBall *ball, BDHalfPlane *plane, float deltaT);
...
//-----------------------------------------------------------------------------------
float bdGetDistanceToBall (float point[2], BallDropBall *ball)
{
	float dx = ball->center.x - point[0];
	float dy = ball->center.y - point[1];
...
//	can be used to find an object within a specified distance from a selection point.
//-----------------------------------------------------------------------------------
float bdGetDistanceToBlock (float point[2], BallDropBlock *block)
{
	float vectorDist[2];
...
//-----------------------------------------------------------------------------------
//	SIMULATING THE MODEL PHYSICS
//-----------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------
//	Advance the state of the specified model by the given time step deltaT
//-----------------------------------------------------------------------------------
void bdAdvanceModelState (BallDropModel *model, float deltaT)
{
	...
	for (j = 0; j < model->numBalls; j++) {
		BallDropBall *ballA = &(model->balls[j]);
		for (i = j+1; i < model->numBalls; i++) {
			BallDropBall *ballB = &(model->balls[i]);
	...
	for (i = 0; i < model->numBalls; i++) {
		float test;
		BDBall *ball = &(model->balls[i]);
...
float *forceDir)
{
	...
	float r = BALL_RADIUS + BLOCK_WIDTH / 2.0f;
